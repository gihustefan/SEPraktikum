...
public interface DecisionGuidanceModelRepository extends GraphRepository<DecisionGuidanceModel> {
    ...
    @Query("MATCH (dgm:DecisionGuidanceModel)-[:HAS_CREATOR]->(user:User) where dgm.published = true optional Match (dgm)-[v:HAS_VOTE]->(:User) where v.vote=true optional Match (dgm)-[w:HAS_VOTE]->(:User) where w.vote=false WITH dgm, user, count(v) as votePositive, count(w) as voteNegative ORDER BY dgm.creationDate DESC SKIP {0} LIMIT {1} RETURN id(dgm) as id, dgm.published as published, dgm.name as name, dgm.description as description, dgm.creationDate as created, dgm.lastModified as modified, user.originalUsername as ownerName, votePositive, voteNegative")
    List<DecisionGuidanceModelDto> findNewestPublishedDecisionGuidanceModels(Integer skip, Integer size);

    //Order by creation date
    @Query("MATCH (dgm:DecisionGuidanceModel)-[:HAS_CREATOR]->(user:User) optional Match (dgm)-[v:HAS_VOTE]->(:User) where v.vote=true optional Match (dgm)-[w:HAS_VOTE]->(:User) where w.vote=false WITH dgm, user, count(v) as votePositive, count(w) as voteNegative ORDER BY dgm.creationDate DESC SKIP {0} LIMIT {1} RETURN id(dgm) as id, dgm.published as published, dgm.name as name, dgm.description as description, dgm.creationDate as created, dgm.lastModified as modified, user.originalUsername as ownerName, votePositive, voteNegative")

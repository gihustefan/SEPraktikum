...
public interface DecisionGuidanceModelRepository extends GraphRepository<DecisionGuidanceModel> {
    ...
    @Query("MATCH (dgm:DecisionGuidanceModel)-[:HAS_CREATOR]->(user:User) WHERE id(user) = {0} AND dgm.published = false optional Match (dgm)-[v:HAS_VOTE]->(:User) where v.vote=true optional Match (dgm)-[w:HAS_VOTE]->(:User) where w.vote=false WITH dgm, user, count(v) as voteTrue, count(w) as voteFalse ORDER BY dgm.creationDate DESC SKIP {1} LIMIT {2} RETURN id(dgm) as id, dgm.published as published, dgm.name as name, dgm.description as description, dgm.creationDate as created, dgm.lastModified as modified, user.originalUsername as ownerName, voteTrue, voteFalse")
    ...
    @Query("MATCH (dgm:DecisionGuidanceModel)-[:HAS_CREATOR]->(user:User) WHERE id(user) = {0} AND dgm.published = false optional Match (dgm)-[v:HAS_VOTE]->(:User) where v.vote=true optional Match (dgm)-[w:HAS_VOTE]->(:User) where w.vote=false WITH dgm, user, count(v) as voteTrue, count(w) as voteFalse ORDER BY dgm.name ASC,dgm.creationDate DESC SKIP {1} LIMIT {2} RETURN id(dgm) as id, dgm.published as published, dgm.name as name, dgm.description as description, dgm.creationDate as created, dgm.lastModified as modified, user.originalUsername as ownerName, voteTrue, voteFalse")

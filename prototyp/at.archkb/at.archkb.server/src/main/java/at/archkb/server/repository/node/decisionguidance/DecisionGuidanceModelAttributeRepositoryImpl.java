package at.archkb.server.repository.node.decisionguidance;
 ...
 */
@Repository
public class DecisionGuidanceModelAttributeRepositoryImpl implements DecisionGuidanceModelAttributeRepository {

    @Autowired
    private Neo4jOperations neo4jOperations;

    @Override
    public <T extends Node> Collection<T> findDGMByTitle(String partialTitle, Class<T> attributeClass) {
    ...
    @Override
    public <T extends Node> T findDGMById(Long id, Class<T> attributeClass) {
        if(id == null)
            return null;

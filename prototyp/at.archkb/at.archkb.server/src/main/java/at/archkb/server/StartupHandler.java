...
public class StartupHandler {
	...
	private void initArchProfiles() {

		ArchProfile archProfile = new ArchProfile();

		archProfile.setDescription(
				"A publicly consumed search solution based on the Velocity Search platform that will help users  findAllByTitle companies of interest.");
		...
		UserDetails userDetails = userBusiness.loadUserByUsername("user@archkb.at");

		Authentication auth = new UsernamePasswordAuthenticationToken(userDetails, userDetails.getPassword(), userDetails.getAuthorities());

		SecurityContextHolder.getContext().setAuthentication(auth);

		archProfileService.createArchProfile(archProfile);
	}
